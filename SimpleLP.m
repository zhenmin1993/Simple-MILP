%Please run ParameterSetting.m before running this script
%Please make sure you have yalmip installed and added to path before
%running
...
for i=1:House_num
    ...
    for j =1:24
        ...
        %When there is power supply, no discharge
        C = [C ; discharge_power(i,j) * Power_supply(j) == 0];
        %When there is power shortage, no charge
        C = [C ; charge_sequence(i,j) * Power_outage(j) == 0];
        %Discharge power equals to the load when outage happens
        C = [C ; discharge_power(i,j) - Load_matrix(i,j)*Power_outage(j)==0];
    end 
end

for j = 1:24
    %Transformer capacity should be higher than the power of 
    %(Load +Battery_charging)
